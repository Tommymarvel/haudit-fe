...
function SidebarLink({
  ...
  return (
    ...
    >
      <span style={{ color: active ? '#7B00D4' : '#5A5A5A' }}>
        {icon}
      </span>
      <span style={{ color: active ? '#7B00D4' : '#5A5A5A' }} className="truncate text-base">{label}</span>
      {badge && unreadCount > 0 && (
    ...
    <div className="mb-1">
      ...
      >
        <div className="flex items-center gap-3">
          <span style={{ color: isActive ? '#7B00D4' : '#5A5A5A' }}>
            {item.icon}
          </span>
          <span style={{ color: isActive ? '#7B00D4' : '#5A5A5A' }} className={cn("truncate text-base")}>
            {item.label}
          </span>
        </div>
        <ChevronDown
